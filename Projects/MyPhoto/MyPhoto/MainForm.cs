...
namespace MyPhoto
{
    public partial class MainForm : Form
    {
        ...
        private bool SaveAndCloseAlbum()
        {
            ...
            {
                // Offerto save the current album
                string msg;
                if (String.IsNullOrEmpty(Manager.FullName))
        ...
        private void SetTitleBar()
        {
            ...
            this.Text = String.Format("{2} - MyPhotos {0:0}.{1:0}", ver.Major, ver.Minor, String.IsNullOrEmpty(name) ? "Untitled" : name);
        }
        ...
        private void mnuAlbumProps_Click(object sender, EventArgs e)
        {
            if (Manager.Album == null)
                return;
            using (menuAlbumProps dlg = new menuAlbumProps(Manager))
            {
                if (dlg.ShowDialog() == DialogResult.OK)
                    DisplayAlbum();
            }
